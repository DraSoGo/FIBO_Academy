...
def measure_from_mask(mask_img, min_area=500):
    ...
    if hole_contours:
        ...
        x, y, w_h, h_h = cv2.boundingRect(largest_hole)
        cv2.rectangle(overlay, (x, y), (x+w_h, y+h_h), (0, 0, 255), 2)  # กรอบแดง
        status = 0
    else:
        status = 1
...
def run_camera(camera_id=0, resize_to_width=None, print_every=5, ppm=None):
    ...
    try:
        while True:
            ...
            overlay, metrics, status = measure_from_mask(mask_img, min_area=500)
            ...
            if frame_i % max(1, int(print_every)) == 0:
                ...
                client.publish(MQTT_TOPIC, json.dumps(out), qos=0)
